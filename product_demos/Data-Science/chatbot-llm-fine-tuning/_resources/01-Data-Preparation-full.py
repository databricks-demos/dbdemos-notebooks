...
# MAGIC %run ./00-init $catalog=dbdemos $db=chatbot $reset_all_data=false
...
#TODO: change that to use mosaicML llama2 endpoint instead
...
# MAGIC %sql select * from training_dataset_question order by doc_id
...
#spark.sql("delete from training_dataset_answer")
for i in range(2500, spark.table("training_dataset_question").count(), 100):
    print(f"processing rows {i}...")
    spark.sql(f'select q.question question, q.id as question_id, d.content from training_dataset_question q inner join databricks_documentation d on q.doc_id = d.id order by question_id asc limit 100 offset {i}').write.mode('overwrite').saveAsTable('databricks_question_chunk_tmp')
    sql_api.execute_sql("""
