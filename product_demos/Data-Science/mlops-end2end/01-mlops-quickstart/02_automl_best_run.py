...
# MAGIC
# MAGIC We have selected the notebook from the best run from the AutoML experiment and reused it to build our model.
# MAGIC
# MAGIC The code below has been automatically generated by AutoML. As a Data Scientist, I can tune it based on my business knowledge if needed.
# MAGIC
# MAGIC For this quickstart demo, we have modified the code so that you can easily re-run the series of notebooks in the demo.
# MAGIC
...
# MAGIC %pip install --quiet databricks-sdk==0.40.0 mlflow==2.22.0 databricks-automl-runtime==0.2.21 holidays==0.64 category_encoders==2.7.0 shap==0.46.0 lightgbm==4.5.0 https://github.com/databricks-demos/dbdemos-resources/raw/refs/heads/main/hyperopt-0.2.8-py3-none-any.whl
...
xp = mlflow.search_experiments(filter_string=f"name LIKE '{xp_path}/dbdemos_automl%'", order_by=["last_update_time DESC"])[0]
...
# MAGIC Configure the search space of parameters. The parameters below are all constant expressions but can be
# MAGIC modified to widen the search space. For example, when training a decision tree classifier, to allow
# MAGIC the maximum tree depth to be either 2 or 3, and set the key of 'max_depth' to
