...
# MAGIC %md 
# MAGIC ## Load dataset for the Lakeflow Pipelines data quality dashboard
...
spark.createDataFrame(expectations, headers.split(",")) \
  ...
  .write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("main__build.dbdemos_retail_c360.dlt_expectations")

# COMMAND ----------

spark.sql(f'alter table main__build.dbdemos_retail_c360.dlt_expectations set tblproperties (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)')
