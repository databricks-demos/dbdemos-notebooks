...
-- MAGIC
-- MAGIC **Your Lakeflow pipeline is ready!** Your pipeline was started using this notebook and is <a dbdemos-pipeline-id="sdp-loans" href="/#joblist/pipelines/460f840c-9ecc-4d19-a661-f60fd3a88297">available here</a>.
-- MAGIC
...
-- MAGIC
-- MAGIC Because these tables will be requested at scale using a SQL Endpoint, we'll add Zorder at the table level to ensure faster queries using `pipelines.autoOptimize.zOrderCols`, and Lakeflow Pipelines will handle the rest.
...
-- MAGIC %md ## Next steps
-- MAGIC
-- MAGIC Your Lakeflow pipeline is ready to be started. <a dbdemos-pipeline-id="sdp-loans" href="/#joblist/pipelines/460f840c-9ecc-4d19-a661-f60fd3a88297">Click here to access the pipeline</a> created for you using this notebook.
-- MAGIC
-- MAGIC To create a new one, Open the Lakeflow Pipelines menu, create a pipeline and select this notebook to run it. To generate sample data, please run the [companion notebook]($./_resources/00-Loan-Data-Generator) (make sure the path where you read and write the data are the same!)
-- MAGIC
