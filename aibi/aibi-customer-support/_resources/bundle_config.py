        ...
        "sql_instructions": [
          {
            ...
            "content": "WITH monthly_ranked_agents AS (SELECT a.`agent_name`, DATE_TRUNC('month', t.`created_time`) AS month, SUM(t.`survey_results`) AS total_survey_results, ROW_NUMBER() OVER (PARTITION BY DATE_TRUNC('month', t.`created_time`) ORDER BY SUM(t.`survey_results`) DESC) AS performance_rank FROM {{CATALOG}}.{{SCHEMA}}.tickets_clean t JOIN {{CATALOG}}.{{SCHEMA}}.agents_clean a ON t.`agent_name` = a.`agent_name` WHERE t.`agent_name` IS NOT NULL GROUP BY a.`agent_name`, DATE_TRUNC('month', t.`created_time`)) SELECT `agent_name`, `month`, `total_survey_results` FROM monthly_ranked_agents WHERE performance_rank <= 10 ORDER BY `month`, performance_rank;"
          },
          {
          ...
          },
          {
            ...
            "content": "SELECT a.`agent_name`, a.`violation_count`, t.`total_tickets`, ROUND((a.`violation_count` :: decimal / t.`total_tickets`) * 100, 2) AS violation_percentage FROM (SELECT t.`agent_name`, COUNT(*) AS violation_count FROM {{CATALOG}}.{{SCHEMA}}.tickets_clean t JOIN {{CATALOG}}.{{SCHEMA}}.sla_clean s ON t.`ticket_id` = s.`ticket_id` WHERE t.`agent_name` IS NOT NULL AND s.`resolution_time` > s.`expected_sla_to_resolve` GROUP BY t.`agent_name`) a JOIN (SELECT t.`agent_name`, COUNT(*) AS total_tickets FROM {{CATALOG}}.{{SCHEMA}}.tickets_clean t JOIN {{CATALOG}}.{{SCHEMA}}.sla_clean s ON t.`ticket_id` = s.`ticket_id` WHERE t.`agent_name` IS NOT NULL AND s.`close_time` IS NOT NULL GROUP BY t.`agent_name`) t ON a.`agent_name` = t.`agent_name` ORDER BY violation_percentage DESC;"
          }
        ],
