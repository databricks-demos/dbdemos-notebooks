...
# MAGIC %md-sandbox
# MAGIC
# MAGIC ## Accelerating credit scoring model creation using MLFlow and Databricks Genie Code
# MAGIC
...
# MAGIC
# MAGIC Instead of creating the same boilerplate for each new project, **Databricks Genie Code** can automatically generate state of the art models for Classifications, Regression, and Forecasting.
# MAGIC
# MAGIC
# MAGIC Models can be directly deployed, or instead leverage the generated code to bootstrap projects with best practices, saving you weeks worth of effort.
# MAGIC
# MAGIC <br style="clear: both">
# MAGIC
# MAGIC ### Using Databricks Genie Code with our Credit Scoring dataset
# MAGIC
# MAGIC All we have to do is describe our problem to Genie Code and point it at the feature table we just created (`creditdecisioning_features`).
# MAGIC
# MAGIC Our prediction target is the `defaulted` column.
# MAGIC
# MAGIC Genie Code generates the full training pipeline and does the rest.

# COMMAND ----------

model_name = "dbdemos_fsi_credit_decisioning"
xp_path = "/Shared/dbdemos/experiments/lakehouse-fsi-credit-decisioning"
xp_name = f"genie_code_credit_{datetime.now().strftime('%Y-%m-%d_%H:%M:%S')}"
# Databricks Genie Code generates a ready-to-use, tracked model from our feature table and
# target column. It creates the full training pipeline for us and logs it to MLflow.
genie_run = DBDemos.create_genie_code_run(f"{xp_path}/{xp_name}", train_df.sample(0.1).toPandas(), model_name=model_name, target_col="defaulted")
...
latest_model = mlflow.register_model(f'runs:/{genie_run.best_trial.mlflow_run_id}/model', f"{catalog}.{db}.{model_name}")
