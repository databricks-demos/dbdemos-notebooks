...
-- MAGIC
-- MAGIC Then, simply manage dependencies and transform data in-place with built-in ETL capabilities on the Databricks Platform (Lakeflow Pipelines), or using your favorite tools like dbt on Databricks SQL for best-in-class performance.
-- MAGIC
...
-- MAGIC
-- MAGIC Avoid expensive queries and materialize your tables. The engine will recompute only what's required when your data get updated. The best way to do so is using Lakeflow Pipelines as shown here: <a dbdemos-pipeline-id="sdp-sql" href="#joblist/pipelines/c8083360-9492-446d-9293-e648527c85eb" target="_blank">Lakeflow pipeline</a> or the [SQL notebook]($../01-Data-ingestion/01.1-SDP-SQL/01.1-SDP-Wind-Turbine-SQL)
