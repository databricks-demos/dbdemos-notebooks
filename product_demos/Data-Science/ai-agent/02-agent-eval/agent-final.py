...
class LangGraphResponsesAgent(ResponsesAgent):
    def __init__(
        self,
        uc_tool_names: Sequence[str] = ("natyra_demo.ai-agent-mcp.*",),
        ...
        if mcp_server_urls:
            ...
            for server_url in mcp_server_urls:
                try:
                    ...
                    tool_defs = loop.run_until_complete(mcp_client.list_tools())
                    print(f" -> Got {len(tool_defs)} tools from {server_url}")
                    ...
                    for tool_def in tool_defs:
                        ...
                        self.tools.append(
                            MCPToolWrapper(
                                tool_def.name,
                                tool_def.description or tool_def.name,
                                server_url,
                                ws_client
                            )
                        )
    ...
    ###########################
    # Required ResponsesAgent methods
    ###########################
...
AGENT = LangGraphResponsesAgent(
    ...
    mcp_server_urls=model_config.get("mcp_server_urls"),   # <-- added
