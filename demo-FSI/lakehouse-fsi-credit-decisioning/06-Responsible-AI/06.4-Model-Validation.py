...
# MAGIC %md
# MAGIC ## Promote Challenger model to Staging
# MAGIC
# MAGIC `TODO`: __Is this paragraph still needed?__ - Our model is now ready. We can review the notebook generated by the auto-ml run and customize if if required.
# MAGIC
...
# MAGIC
# MAGIC If `challenger model` becomes the new candidate `champion model`, we promote it to `Staging`, where it will go through integration testing, before being deployed to `Prodution`. Otheriwse, we archive the `challenger model` and no further changes are needed.
# MAGIC
# MAGIC Before promoting the model into `Staging`, there can be a human validation of the model involved.

# COMMAND ----------

## TODO: Do we still need this for validation status?
...
if champion_model_info is None:
  # No champion model. Set this model as the candidate champion by promoting it to Staging
  client.delete_registered_model_alias(name=f"{catalog}.{db}.{model_name}", alias="None")
  client.set_registered_model_alias(name=f"{catalog}.{db}.{model_name}", alias="Staging", version=challenger_model_info.version)
  print(f'{challenger_model_info.version} of {catalog}.{db}.{model_name} is now promoted to Staging.')
elif challenger_run.data.metrics['val_f1_score'] > champion_run.data.metrics['val_f1_score']:
  # Challenger model is better than champion model. Promote challenger model to Staging.
  ...
  # Challenger model is not better than champion model. Set challenger model as Archived
  #client.delete_registered_model_alias(name=f"{catalog}.{db}.{model_name}", alias="Staging")
  client.delete_registered_model_alias(name=f"{catalog}.{db}.{model_name}", alias="None")
  client.set_registered_model_alias(name=f"{catalog}.{db}.{model_name}", alias="Archived", version=challenger_model_info.version)
  print(f'{challenger_model_info.version} of {catalog}.{db}.{model_name} is transitioned to Archived. No model promoted to Staging.')
...
# MAGIC # `TODO`
# MAGIC
# MAGIC - Check/add logic to promote model to Staging
