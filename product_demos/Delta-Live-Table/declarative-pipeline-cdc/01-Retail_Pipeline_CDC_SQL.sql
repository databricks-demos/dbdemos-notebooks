...
-- MAGIC %md
-- MAGIC
-- MAGIC # Implement CDC with Lakeflow Pipelines
-- MAGIC
...
-- MAGIC %md
-- MAGIC ### Materializing table from CDC events with Lakeflow Pipelines
-- MAGIC
...
-- MAGIC - Extra: we'll also see how Lakeflow Pipelines can simply create Slowly Changing Dimention of type 2 (SCD2) to keep track of all the changes 
-- MAGIC
...
-- MAGIC
-- MAGIC ## Accessing the Lakeflow Pipelines
-- MAGIC
-- MAGIC Your pipeline has been created! You can directly access the <a dbdemos-pipeline-id="sdp-cdc" href="/#joblist/pipelines/c1ccc647-74e6-4754-9c61-6f2691456a73">Lakeflow Pipelines for CDC</a>.
...
-- MAGIC
-- MAGIC Open [/1-sdp-sql/transformations/01-sql_cdc_pipeline.sql]($./1-sdp-sql/transformations/01-sql_cdc_pipeline.sql) to get started and explore how to do CDC with Lakeflow Pipelines!
-- MAGIC
...
-- MAGIC
-- MAGIC Thanksfully Lakeflow Pipelines solves these challenges out of the box with the `APPLY CHANGE` operation
...
-- MAGIC
-- MAGIC #### SCD2 with Lakeflow Pipelines
-- MAGIC
...
-- MAGIC %md
-- MAGIC ### Conclusion 
-- MAGIC We now have <a dbdemos-pipeline-id="sdp-cdc" href="/#joblist/pipelines/c1ccc647-74e6-4754-9c61-6f2691456a73">our Lakeflow pipeline</a> up & ready! Our `customers` table is materialize and we can start building BI report to analyze and improve our business. It also open the door to Data Science and ML use-cases such as customer churn, segmentation etc.
...
-- MAGIC
-- MAGIC Lakeflow Pipelines engine will handle the parallelization whenever possible, and autoscale based on your data volume.
-- MAGIC
