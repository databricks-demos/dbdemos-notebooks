...
# MAGIC %uv pip install --quiet -U mlflow[databricks] lxml==4.9.3 transformers==4.49.0 langchain==0.3.25 databricks-vectorsearch==0.55 bs4==0.0.2 markdownify==0.14.1
...
# MAGIC
# MAGIC *Note that this part would typically be setup as a production-grade job, running as soon as a new documentation page is updated. <br/> This could be setup as a Lakeflow pipeline to incrementally consume updates.*
...
# MAGIC
# MAGIC That's all we have to do. Databricks will automatically capture and synchronize new entries in your Lakeflow Pipelines.
# MAGIC
